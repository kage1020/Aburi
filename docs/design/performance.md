...
The corpus above does not exist yet. Nine public repositories at pinned commits do, under [`benchmarks/public-repos/`](https://github.com/kage1020/Aburi/blob/main/benchmarks/public-repos/README.md), and they carry the same two numbers. On a 4 vCPU / 16 GiB runner, single-threaded, with no pool at all, the 2026-09-01 sweep finished 1,081 files in 3.7 s, 1,914 in 3.9 s and 5,002 in 14.7 s, and peak RSS never passed 466 MiB. Cost is linear in file count: a least-squares fit over the nine gives roughly 2.8 ms per file plus 326 ms of fixed grammar-load and config cost.
...
PF11 has its single-threaded half confirmed on the way: three consecutive `aburi scan --no-timestamp` runs over an unchanged tree produced byte-identical IR on all nine repositories. What remains unmeasured is the equality *across* `--concurrency` values, which is the half only the pool can exercise.
