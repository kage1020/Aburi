...
The identity pass runs behind a shape gate, and both exist for the same reason. `buildDiff` calls `checkDocumentShape` — invariant #20, and only #20 — on each side first, raising `ir-shape-invalid` naming the collection, the index and the field. Without it, a Symbol missing `fingerprint`, `source`, `calls`, `decorators`, `effects` or `rules` crashed a matching stage with a `TypeError` that named none of the three, and a Component missing `roots` did the same in §6.1. The gate is not scoped to the fields the matcher happens to dereference: what it establishes is what the `IR` brand asserts, so a scope that moved with the matcher would leave a caller's Document conditionally valid. With it in place, the identity pass's own type guards are a local guarantee rather than the live path.
