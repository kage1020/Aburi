...
The fold is over the *bare* name's id, not over the raw string: `@acme/---` yields the empty id and aborts as the rule below says, rather than kebab-casing to the scope alone and giving every unusable name in one scope the same silent id. It also erases the namespace boundary, so `@foo/bar-baz`, `@foo-bar/baz` and an unscoped `foo-bar-baz` are one id where the first two were distinct; that is a collision like any other and the rule below resolves it.

On collision (multiple workspaces yielding the same id) → append the parent directory name as a suffix (`billing` → `billing-apps` / `billing-packages`). Where that is still not enough — `team1/shared/pkg` and `team2/shared/pkg` both suffix to `pkg-shared` — the suffix takes one more step up the path for every id still shared, until each is unique or its root has no more ancestors (`pkg-shared-team1` / `pkg-shared-team2`). A suffix that lands on another, already-unique id puts that id in the next round too, so it moves as well. What is left after the path runs out — two roots whose ancestor segments all kebab-case to nothing, or a group in which every member has consumed its whole path — takes a short hash of `roots[0]` **appended to the id it reached** (`utils-packages-b9b98f98`), every member of the group included. A root with no ancestors at all, `.`, is only in that position when the collision outlasts every other member's path too: `.` against `packages/foo` resolves to `app` / `app-packages` with no hash.

Two things this does and does not promise, because the difference is the whole point of the rule:

- **It never reads a component's position.** The suffix is built from that component's own base id and `roots[0]`, and how far up the path it goes is decided by the *set* of ids in contention — not by the order the components arrive in, and not by which of them sorted first. The positional `-2`, `-3`, … counter it replaces did exactly that: adding `@alpha/utils` ahead of `@beta/utils` and `@gamma/utils` demoted both of them by one, and since the id keys `Symbol.component`, both `Dependency` endpoints and cross-revision comparison, a single new package read downstream as every component having been replaced.
- **It does not promise an id never moves.** A package that arrives claiming an id already in use moves whoever holds it: adding `packages/x` named `shared-libs` takes `team/libs/shared` from `shared-libs` to `shared-libs-team`. What has changed is the reach of that — before, *any* package under the same parent renumbered its neighbours; now only a package contending for the same id moves anything, and components elsewhere in the tree are untouched.

Hash suffixes separate a group by digest rather than by construction, so uniqueness is checked once on exit. A duplicate that survives — no run has produced one — aborts with `component-id-collision-unresolved` rather than being handed back: `aburi init` writes `components[]` without building an IR, so the §14 #2 uniqueness invariant would not be the one to catch it.
...
| CD8 | 2 workspaces generate the same id (both "shared") | Suffixes appended ("shared-apps" / "shared-packages"). Adding a third workspace elsewhere moves them only if it contends for one of those ids; where the path separates nothing, each id ends in a digest of its own `roots[0]` and adding a fourth leaves the first three byte-identical |
