...
- If any plugin call for a file throws — `parseFile`, `extractSymbols`, `walkBody`, `normalizeAst`, a framework `classifySymbol`, or an effect `classify` — the core withdraws **that file** and keeps going. One file's bug does not halt IR generation.
...
- **One exception is not absorbed.** A `CoreError` whose code is `scan-plugin-misconfigured` says the plugin *set* is wrong rather than this file — an effect plugin returning a Promise from `classify`, a language plugin emitting Symbol ids with no language prefix. Both repeat for every file, so it propagates and ends the run. Every other coded error describes the file (`anonymous-symbol-id-attempted` from a qualified name the grammar refuses, `non-posix-path` from where the file lives) and is absorbed.
- A plugin-wide bug outside that one code therefore presents as one failure per file rather than one crash. That is the intended shape: every file is named, the messages are identical, and the count is the whole workspace.
- `config.strict` is reserved for a "stop at the first extraction exception" mode. No CLI flag defines it and nothing reads it today, so the behaviour above is unconditional.
