...
**The path segment of `id` must be the path of the file being extracted** — `ctx.file.path`, the same value the candidate's `source.file` carries. The grammar cannot enforce this: another file's path is a perfectly legal path, so `makeSymbolId` builds the id and the fault only shows as a Symbol filed under a file that never declared it. A plugin re-exporting or generating code names the declaring file in `source.file` and the declaring file alone, rather than the file it happened to read. The scan checks this per file and withdraws the file that broke it (§7.2).
...
A Symbol id names an entity, not a declaration, and most languages let one entity be written more than once: a getter beside its setter, an overload beside its implementation, an interface reopened, a namespace augmenting the class above it, a partial class. **A plugin emits one SymbolCandidate for all of them.** Two candidates sharing an id costs the file they were written in: [ir-schema.md](./ir-schema.md) §14 invariant #1 is decided per file, inside the boundary of §7.2, so the file is withdrawn and named and the rest of the workspace is scanned. It is still a file this plugin could have extracted, so folding is the plugin's job either way.
...
Nothing in the core checks that a plugin folded its declarations — the invariant catches the omission, at the file that wrote it. `derivedBy` is where a plugin says a fold happened; the in-tree TypeScript plugin writes `declaration-merged`.
...
segment ([`ir-schema.md`](./ir-schema.md) §3.1) folds into the target as that segment**, a
private name aside (below). The literal is decoded rather than unquoted, so `prisma["us\u0065r"]` is
...
A string that opens with `#` names no segment either, for the opposite reason: the grammar has
the segment `#v`, but it names the `#`-private member (LP8o), and `obj["#v"]` is the public
property with those characters. Core's `isQnameSegment` refuses `#v` unless asked for a private
name, and a decoded string never asks.
...
the program, it is spelled exactly like an ordinary two-segment method call, and a consumer that
counts segments — `effects-prisma` needs three for a delegate call — loses the `db.write` with
nothing anywhere to say that it did. Recording no call at all would at least be honest, and it
...
- **A file whose Symbols cannot enter the Document is withdrawn on the same terms**, though nothing threw. Today that is ir-schema.md invariant #1, read off the file being extracted: two of its own Symbols under one id, or one whose id names a path that is not this file's (§4.3). Both are the fault of the file being scanned, so that is the file that goes. The check runs per file, before any of the file's output is accumulated, because that is the last point at which the file can still be withdrawn and named — `assertIRIntegrity` sees the assembled Document and can only refuse the whole of it, which is what used to happen. Which of two Symbols to keep is not the core's to decide — the plugin reported nothing that separates them — so the file goes whole, with `code: "duplicate-symbol-id"` on its `ScanResult.extractionFailures` entry and the collision named in `detail`. The document-wide check stays where it is, as the backstop.
...
- A file withdrawn because a plugin *threw* loses its recoverable parse errors, unlike one abandoned for its `parseTimeoutMs` budget: the pipeline result never materialized, so the thrown message is the diagnostic in their place. A file withdrawn for a duplicate id keeps them, on the same reasoning read the other way — its extraction ran to a result, so those errors are observations the parse really made about the file, and dropping them would hide a genuine parse fault behind an id fault that may well be its consequence.
...
- The behaviour above is unconditional: there is no "stop at the first extraction exception" mode. (`config.strict` is about something else: whether an undeclared value ends the run, which it does through `vocab-undeclared` above.)
...
| LP4 | class static method, or an export of a namespace merged into the class (LP8n) | name = `Class::method` |
| LP5 | interface (TS/Java/Go) | SymbolCandidate.kind = "interface" |
| LP6 | default export of anonymous function | name = `<default>` |
| LP6a | a default export written **apart** from the declaration it names — `const Page = () => …` followed by `export default Page` | the declaration's own SymbolCandidate, `visibility = "public"` and `derivedBy` carrying `export-default`, exactly as the wrapped spelling (`export default function Page()`) reports. The two are one boundary and the frameworks that read `export-default` to find a page or a component must not depend on which was written — which is also why the value is read through LP7a's wrappers, `export default Page satisfies NextPage` being an ordinary spelling of the same export. What the reading stops at is what is not a reference to a declaration: a call (`withAuth(Page)`, by LP7b), a value the module builds (`{ Page }`), a member of one (`Routes.Page`), and an identifier that names an import, which declares nothing here to reach. A declaration reached by name is reached by a **bare** name: a class member called `Page` (`Shell.Page`, `Shell::Page`) and a namespaced one (`Routes.Page`) carry a separator no bare identifier can spell, and a call Symbol (LP20g), whose qname is one identifier-legal segment and so has no separator to rely on, is refused by kind. `export { Page as default }` is an export clause, and no clause spelling — `export { Page }` included — is read for visibility yet; covering only the `default` one would make the plugin's answer depend on the clause's contents |
| LP6b | a declaration carrying the `export` keyword, of **any** kind — `export interface I {}` beside `export const x = 1` | `visibility = "public"` **and** `derivedBy` carrying `export-keyword`, for every kind alike. Whether a declaration was exported is a question about the statement it was written as, so the answer cannot depend on which declaration follows the keyword — and for four kinds it did: the interface, type alias, enum and namespace builders wrote a one-token `derivedBy` naming the kind and stopped there, where `visibility` was right throughout. The cost was not a missing field but a spelling-dependent answer: a consumer asking a Symbol "was this exported?" was told so for the `const` and told nothing at all for the `interface` beside it. `derivedBy` is the closed vocabulary diffs and filters read (§4.3, [ir-schema.md](./ir-schema.md) §5.5) and LP6a already puts exportedness in it — a framework plugin finds a page by reading `export-default` off a Symbol — so the token is evidence about the declaration, not a note about how the module was written. **One** reader answers it for every kind, for LP7a's reason: the node it is handed differs per kind (the `module` for a namespace, the enclosing `lexical_declaration` or `variable_declaration` for a variable, the declaration itself for the rest), and a second reader is where the kinds start to disagree again. It reaches the statement through the `declare` wrapper, so `export declare interface I {}` answers as LP36 has it. **Within one statement** `export default` replaces the keyword rather than joining it — the statement cannot be written with both, and LP6a's token is the one a framework reads; across two statements they join, LP6a's detached spelling carrying `export-keyword` and `export-default` alike, so exclusivity is a rule about a statement and never about a Symbol. Illegal source the grammar still accepts answers rather than crashing: `export default const x = 1` reports the default export, as the same two keywords on a class do. A declaration written inside an exported namespace is not exported by it, and one written `export` inside a plain namespace is: the keyword is read off the declaration's own statement, which is what `visibility` answers too. No export **clause** spelling is read here either, `export { I }` included, for the reason LP6a gives. Where the two answers can come apart is a Symbol several declarations wrote, and only in source the language rejects: §4.3.1 gives `visibility` from the leading declaration and `derivedBy` from the union, so `interface I {}` reopened as `export interface I {}` (TS2395) is `internal` and carries the token. Legal source cannot reach it — a merge's declarations must agree about being exported — which is why the fold keeps one rule for every scalar rather than deriving this one from the union |
| LP7 | `const f = () => ...` | name = `f` (the variable name becomes the qname) |
| LP7a | the same binding written behind a wrapper that says nothing about the value — TypeScript's `(f)`, `f as T`, `f satisfies T`, `f!` | the same answer as LP7. A wrapper the language uses to group, to name a type or to assert non-null does not replace the value, so the test for "is this a function?" reads through it. **One** reader answers what a wrapper is, for every question the plugin asks about a node — a binding, a member, a call's argument, a call's receiver and a default export's value (LP6a). Two readers is the defect, not the duplication: while the receiver side hand-unwrapped only parentheses, `(app as Express).get(p, h)` was not a registration at all |
...
| LP8h | a declaration merged with an earlier one — a reopened interface or namespace, a namespace augmenting the class or function above it | one SymbolCandidate, carrying the earlier declaration's kind and range and **every** declaration's `derivedBy`. Anything the later declaration declares in turn (`N.b` from a reopened `namespace N`, `f.b` under a function) is its own Symbol and must still be there; under a class it is named by LP8n |
| LP8n | what a namespace merged into a class declares — `class C {}` beside `namespace C { export function m() {} }` | an export is the class's static member, `C::m`, apart from an instance member `C.m`; everything under it sits under that segment (`C::Inner.g`, and `C::K::s` for a nested class's static). A statement the namespace does not export is local to it and keeps the dot (`C.local`, `internal`). An ambient namespace exports without the keyword, and a dotted `namespace C.D {}` makes `D` the member (`C::D.x`). The class is looked for in the statement list the namespace is written in, so one declared in another block of a reopened outer namespace is not seen and the exports keep the dot (`A.C.x`) |
...
| LP8m | a member whose name is written as a **string literal** — `class C { "createInvoice"() {} }` | the segment the literal *decodes* to, when that is an identifier the qualified-name grammar admits; otherwise **no** SymbolCandidate (LP20c). A decoded `"#v"` has no SymbolCandidate either: the segment `#v` is the `#`-private member's (LP8o), and a quoted key is a public property whatever its characters, so the producer refuses the leading `#` — the refusal is the producer's, not the grammar's, and it is the default `isQnameSegment` answers with. A property key is a string, so the quoted and the bare spelling of one identifier are one member and fold by id (§4.3.1) — and `"constructor"() {}` is the construction path, which the language decides by the property name and not by how it is written. A name the parser guessed at is refused in both shapes it arrives in: a literal that parsed in part, and one that did not parse at all — the second leaves no literal to refuse, so what says so is an ERROR among the member's **own** children, never `hasError`, which a broken body would trip |
| LP8o | a `#`-private member — `class C { #v() {} static #w() {} }`, a `#`-private field holding a function, accessor pair | `C.#v` and `C::#w`, with the `#`, and `visibility = "private"`. `#v` and a `v` written beside it are two members, so dropping the `#` folds two entities onto one id (§4.3.1). The segment is ECMAScript's PrivateIdentifier: one `#`, leading, and only after a separator ([`ir-schema.md`](./ir-schema.md) §3.1). Only the private name **node** asks the id builder for it — core's `isQnameSegment` admits `#v` only with `privateName` set — so a string key cannot reach it (LP8m, LP20j). Visibility is read off the node type rather than the segment's `#`, which carries it only because the string refusal holds. `#constructor` has its own segment and is an ordinary member, never the construction path |
...
| LP35 | a member declared `abstract` — `abstract class A { abstract doIt(): void }` | a member SymbolCandidate, `kind = "method"`, `bodyNode` null, `derivedBy` carrying `abstract-declaration`. The language forbids an implementation beside it, so nothing else in the class declares the member, and skipping it left an abstract class reporting only the methods it happened to implement — where the declared one is the API the class promises and what a subclass is checked against. An `abstract` accessor pair is LP8a's fold. Abstract overloads fold too, but LP8f's rule cannot apply as written — it takes the signature from the *implementation*, and an abstract pair has none — so the Symbol keeps the **leading** declaration's signature. An abstract **field** is a field, on LP20f's terms: it holds no function |
| LP36 | a declaration written under `declare` — `declare function f(): void`, `export declare class C { m(): void }`, `declare namespace N { … }`, `declare const` / `enum` / `interface` / `type` | the SymbolCandidate the same declaration gets written without the keyword, plus `ambient-declaration` on `derivedBy`, and the members and nested declarations under it. `declare` is a modifier the grammar spells as a wrapper node, so it is read *through* rather than matched on — including between `export` and the declaration, where a reader taking the immediate parent finds the wrapper rather than the export, and above it, where the JSDoc is written. **One divergence is recorded rather than closed**: an ambient namespace exports its members whether or not they carry the keyword — `declare namespace N { function h(): void }` makes `N.h` resolvable, and only an `export {}` or `export =` in the body switches that off — where `computeTopLevelVisibility` looks for an `export_statement` parent and so answers `internal` for `h`. Closing it needs visibility to be a question about the enclosing scope rather than about the statement, which nothing else in this table asks for. `.d.ts` files are dropped before extraction ([drop-list.md](./drop-list.md) Category A); these forms are written in ordinary `.ts` files and are not |
| LP36a | a **signature** with no body — `function f(): void`, `class C { m(): void }` | a SymbolCandidate only under a `declare` (LP36). Written outside one it is an overload declaration, and the implementation beside it carries the body and the parameter types the entity is actually called with, so the implementation is the Symbol and the signature is not — the answer a top-level overload and a class member have to give alike, or the same source answers differently depending on where it is written. An ambient context has no implementations at all, so there is nothing beside the signature to defer to, and reading its members as overloads left `export declare class` with no methods |
| LP36b | a `declare` that augments a scope other than this module — `declare global { … }`, `declare module "express" { … }` | **no** SymbolCandidate, for the augmentation or for anything inside it, and no diagnostic. What either declares is a member of the global scope or of another module, and the only qualified name available is a top-level one — which would claim that name for this file and fold with the file's own declaration of it (§4.3.1). The specifier is not a qualified-name segment either, so building an id from it throws and costs the file every Symbol it had (§4.3). A known limit: covering these needs a qname convention for a scope that is not the file's, which [ir-schema.md](./ir-schema.md) §3.2 does not have |
...
| LP14d | a receiver that is not a plain path — `@arr[0].C()`, `@(a).C()`, `@pick().C()`, `@ns["C"]()` | decorators = [], so there is nothing to qualify. Not because the grammar refuses them: each parses as a decorator of the fragment the grammar could take, inside an ERROR node that leaves it no longer a preceding sibling of the declaration, which is what puts it out of the run. A plugin must not read this as a grammar guarantee — it is where error recovery happens to put the node |
| LP14e | a receiver the grammar reaches but that binds nothing — `@this.C()` (parses cleanly), `@a?.C()` (recovered, the `?` an ERROR child of the member expression) | the object verbatim: `this`, and `a`. The extractor quotes what was written; a consumer matching it against an import edge finds nothing and falls in the unbound row of §5.2.2 |
| LP14f | a path written in parentheses — `@(a.b)` | name = "b", qualifier = "a", as for `@a.b`. The rule is the extractor's own and not the grammar's: a qualifier is reported when what the parentheses enclose is a member expression with an object, and in no other case |
| LP14g | a name, a path or a call in parentheses, which the grammar parses cleanly — `@(Controller)`, `@(nest\n  .Controller)`, `@(Controller('/x'))` — or with a repair only inside the argument list, `@(Controller(a b))` | read through the parentheses: `Controller`, `Controller` with qualifier `nest`, and `Controller` with arguments `["'/x'"]`, so each matches the vocabulary its unparenthesized form does. A comment inside is skipped. `raw` quotes the parentheses, and a line break inside them stays in `raw` and out of `name` |
| LP14h | a parenthesized expression the grammar had to repair — `@(x as any)`, `@(x!)`, `@(a[b])`, `@(C<T>)`, `@(new C())`, `@(a, b)`, `@(pick().C)`, `@(a?.C)`, `@(nest.)` | one decorator, **`name: "<expression>"`** (`UNNAMED_DECORATOR` in `@aburi/types`), no qualifier, no arguments, the text in `raw`. TypeScript accepts any expression there and the grammar's decorator rule does not, so the parse holds an ERROR node beside a fragment (`x`, `a`, a call of `new`) that is not the decorator. The repair that counts is one in the head, an ERROR or MISSING token among the children of the parentheses or of what they enclose; a broken argument list nests deeper, so `@(Controller(a b))` is `Controller`, as `@Controller(a b)` is (LP14g). `@(a?.C)` is the one form whose unparenthesized spelling reads differently: LP14e names `@a?.C()` `C` with qualifier `a`, from the same repaired member expression. The difference is this row's choice, since a repair inside parentheses is refused wherever it sits. `Decorator.name` is never a node's text: it is an identifier or this marker, which begins with `<` and so matches no vocabulary |
...
| LP20c | a member with **no** Symbol of its own — a computed name, a name the qualified-name grammar has no segment for (`"a-b"() {}`, `1() {}`), a quoted `"#v"` (LP8m), a member of an anonymous default export | its body stays on the owner. Skipping it would lose the calls in it with no Symbol, no diagnostic and nothing to say so. The plugin needs one predicate for "is this member a Symbol?" that both extraction and the walk read — and both must ask it about the **same owner node**. The walk reads the owner off the body it is walking, not off the Symbol: a folded Symbol's `fullNode` is its leading declaration (§4.3.1) and need not be the owner at all |
| LP20d | a function's parameter defaults — `function f(x = g())`, `(y = k()) => l()`, `m(x = f()) {}`, an inline handler's | on the function's own Symbol, walked with its body and ahead of it. A default runs on every call that omits its argument, so it is what calling the function runs; outside a class there is no other Symbol to carry it, and a default not walked here is in the IR nowhere. For a member the owner then skips the parameter list as it skips the body (LP20a), so the default is not reported twice — except the constructor's, which the owner keeps whole under LP20b, so it is on both. A decorator is not a default and stays on the owner, whether it sits beside the member (a method's, a sibling in tree-sitter-typescript), inside the field, or on a parameter (`m(@Body(pipe()) x)`): a decorator's arguments run where the decorator is applied, when the class is defined |
...
| LP20j | `prisma["user"].create(d)`, `prisma.user["create"](d)` | one call each, target `prisma.user.create`, no `dynamicReceiver` — a string-literal index is the segment it spells, in the receiver slot and in the terminal one (§4.4), unless that segment is a private name (LP20k) |
...
| LP27a | a bare `&` in JSX — `<p>Subscription & Billing</p>`, `href="/x?a=1&b=2"` | no parse error. The tsx grammar reads `&` as the opening of an HTML character reference and errors when no `;` closes it, which makes prose and a tracking URL indistinguishable from a broken file; the plugin drops that one shape so the recoverable-error count stays worth reading. Narrow on purpose: the run has to open with an ampersand-led token, sit among a JSX element's children or inside a JSX attribute's string, and — among children — hold none of `{`, `}`, `<`, `>`, which JSX text cannot contain. The last rule is what stops an `&` swallowing a real break merged into the same ERROR node, as in `<div>a & } b</div>`; it does not apply inside an attribute's string, where all four are ordinary. A truncation whose ERROR sits outside both positions still reports, including in a file that carries both |
| LP27b | an `import("…")` type the grammar reads as a call and cannot place — first in a call's type arguments (`importActual<typeof import("./m")>()`, the vitest mocking idiom) or before `[]` (`import("./m").Rule[]`) | no parse error, and the declarations after it survive, including a run of them that recovery reads as the whole file broken. The file is parsed a second time with each such `import(…)` replaced by an identifier of the same length, and the tree reads the original text, so no Symbol name, call target or `node.text` shows the replacement. Node kinds are the second parse's: the `import(…)` is an `identifier` there, so it adds no `import` call target (a clean `typeof import("./m")` in a body adds one), and the masked shape is what `normalizeAst` serialises. Its import edge is kept, so type-only imports produce edges here as LP26g says. Only an `import(…)` with a non-empty string specifier, on one line, in the nearest statement, declaration, member definition or signature holding an error, or inside an ERROR node, is replaced, so a type alias that parsed keeps its tree and `import("")` keeps its empty-specifier error; a replacement the second parse does not read as a type is withdrawn, so a dynamic `import("./x")` in the same statement stays a dynamic import; and the second tree is used only when it has fewer errors. When the second parse throws, the first tree is kept and the failure is one more recoverable error |
...
| LP28b | a plugin emitting two Symbols under one id for one file | that file is withdrawn with `reason: "extraction-failed"` and `code: "duplicate-symbol-id"`, naming the id and both declarations' lines; every other file still reaches the Document, and the run exits `3` like any other extraction failure. The file keeps its recoverable parse errors, unlike one a plugin threw on (§7.2) |
| LP28c | a plugin emitting a Symbol whose id names a file other than the one being extracted (§4.3) | the file that *wrote* the id is withdrawn on LP28b's terms, whichever of the two discovery reached first, and the message names the path the id claims against the file it came from. Withdrawing the file the id names would take a healthy file for another's fault |
