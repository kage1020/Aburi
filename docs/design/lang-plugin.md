...
| LP14f | a receiver the grammar admits that is not a path at all — `@(a.b)` | one decorator, `name: "(a.b)"` and **no qualifier**. The rule is the extractor's own and not the grammar's: a qualifier is reported when the callee is a member expression with an object, and in no other case |
