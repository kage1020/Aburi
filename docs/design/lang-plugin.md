...
| LP14g | a name, a path or a call in parentheses, which the grammar parses cleanly — `@(Controller)`, `@(nest\n  .Controller)`, `@(Controller('/x'))` | read through the parentheses: `Controller`, `Controller` with qualifier `nest`, and `Controller` with arguments `["'/x'"]`, so each matches the vocabulary its unparenthesized form does. A comment inside is skipped. `raw` quotes the parentheses, and a line break inside them stays in `raw` and out of `name` |
| LP14h | a parenthesized expression the grammar had to repair — `@(x as any)`, `@(x!)`, `@(a[b])`, `@(C<T>)`, `@(new C())`, `@(a, b)`, `@(pick().C)`, `@(a?.C)` | one decorator, **`name: "<expression>"`** (`UNNAMED_DECORATOR` in `@aburi/types`), no qualifier, no arguments, the text in `raw`. TypeScript accepts any expression there and the grammar's decorator rule does not, so the parse holds an ERROR node beside a fragment (`x`, `a`, a call of `new`) that is not the decorator. `Decorator.name` is never a node's text: it is an identifier or this marker, which begins with `<` and so matches no vocabulary |
