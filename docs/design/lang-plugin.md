...
| LP27b | an `import("…")` type the grammar reads as a call and cannot place — first in a call's type arguments (`importActual<typeof import("./m")>()`, the vitest mocking idiom) or before `[]` (`import("./m").Rule[]`) | no parse error, and the declarations after it survive. The file is parsed a second time with each such `import(…)` replaced by an identifier of the same length, and the tree reads the original text, so no Symbol, call or `node.text` shows the replacement. Only an `import(…)` in a statement holding an error, or inside an ERROR node, is replaced, so a type alias that parsed keeps its tree; a replacement the second parse does not read as a type is withdrawn, so a dynamic `import("./x")` in the same statement stays a dynamic import; and the second tree is used only when it has fewer errors |
