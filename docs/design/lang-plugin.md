...
| LP20d | a member's parameter defaults | on the owner. They sit outside the member's own `bodyNode`, which is its body block, so skipping the whole member rather than its body loses them. A decorator may or may not be inside the member node — in tree-sitter-typescript it is a sibling, so it is a member in its own right and this rule never reaches it |
