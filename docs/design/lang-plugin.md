...
| LP28b | a plugin emitting two Symbols under one id for one file | that file is withdrawn with `reason: "extraction-failed"` and `code: "duplicate-symbol-id"`, naming the id and both declarations' lines; every other file still reaches the Document, and the run exits `3` like any other extraction failure |
