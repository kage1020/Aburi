...
| LP7a | the same binding written behind a wrapper that says nothing about the value — TypeScript's `(f)`, `f as T`, `f satisfies T`, `f!` | the same answer as LP7. A wrapper the language uses to group, to name a type or to assert non-null does not replace the value, so the test for “is this a function” reads through it. One reader answers this for the plugin, so a binding, a member and an argument cannot disagree |
