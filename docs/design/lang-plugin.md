...
| LP26f | `import x = require('./m')` | one edge, `{source: "./m", symbols: "*", line: 1, dynamic: false, namespaceBinding: "x"}`. `x` binds the module object as `import * as x` does, so the namespace shape is what lets call resolution strip the head off `x.foo()` and look for `foo` in the target; a default binding (`symbols: ["x"]`) would send it looking for `x.foo` there instead. `dynamic` is false because both resolution loops skip a dynamic edge |
