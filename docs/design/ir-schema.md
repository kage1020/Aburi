...
- `<file-path>`: POSIX path relative to the workspace root (forward slashes enforced). Non-empty, never absolute, and never ascending out of the workspace via `..` — the same rule §14 invariant #10 applies to every other path in the Document. It also holds neither `:` nor `#`, because the id is split on the first of each
- `<qualified-name>`: name unique within the file. `.` and `::` are separators that join two named constructs, so every segment they delimit is a non-empty identifier: `A.`, `.A`, `A..B` and `::` are not qualified names. `<default>` (§3.3) is the one reserved exception to the identifier rule
...
10. Every path — `symbols[].source.file`, `components[].roots[]`, `workspace.managers[].roots[]` — is non-empty, POSIX (forward slash, no backslash), not absolute, and free of `..` segments. The `..` rule is not cosmetic: a consumer reading a Document off disk resolves these paths against the filesystem, so a `source.file` of `../../../../etc/passwd.ts` would point outside the workspace the Document claims to describe. The rule is the one `makeSymbolId` applies on the way out, shared rather than restated, so the paths Aburi writes and the paths it accepts are one set
