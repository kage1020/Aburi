...
`aburi diff --max-bytes <n>` caps `diff.md` at n UTF-8 bytes, least-important-first: a section of
whole symbols is first cut to one name-and-location line per symbol, and a section is dropped only
when that does not fit. A note under the Summary names both kinds — a byte cut would leave a
