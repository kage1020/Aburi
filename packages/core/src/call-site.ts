...
/** Identity of a `Dependency`: the `(from, to, via)` triple ir-schema.md §14 #13 makes unique. */
export function dependencyKey(from: string, to: string, via: string): string {
  return [from, to, via].join(CALL_SITE_KEY_SEPARATOR)
}

/** Identity of a call-graph edge, where `via` is fixed to `"call"` (ir-schema.md §14 #14). */
