...
import { IR_SCHEMA, type PluginLineup, type ScanExtras, scanWith } from "../src/scan-helper"
...
/**
 * Validating a generated IR against `schema/aburi.ir.v1.json` is what puts the conditional
 * `Effect` constraints and `inferredThrows`' `minItems` in front of a real document rather
 * than a hand-built fixture. No path is excluded, `workspace.languages` included.
 */
...
describe("scan — integration through real plugins", () => {
  ...
  it("dedupes multi-line calls to the same callee into a single via:call Dependency", async () => {
    ...
    const caller = result.ir.symbols.find((symbol) => symbol.name === "caller")
    // Three per-line Call entries survive on the Symbol side (integrity #11 requires
  ...
  it("emits every Class A key in the serialized IR (ir-schema.md)", async () => {
    ...
    // and a document on disk that is missing the key.
    ...
    for (const component of parsed.components) {
      ...
      // Class B on the same record: the empty case is an absent key, not `[]`. Phrased as
      // "present implies non-empty" rather than "always absent" so that a fixture gaining a
      // detectable framework does not fail a test about key presence.
  ...
  it("reads an IR whose Class A keys were never written (ir-schema.md reader rule)", async () => {
    ...
    // conditional on the value being `null`.
    ...
    // against the emitted head finds nothing changed.
  ...
  it("writes a canonical JSON IR to disk via writeCanonicalIR", async () => {
    ...
    expect(parsed.$schema).toBe(IR_SCHEMA)
  })
})
