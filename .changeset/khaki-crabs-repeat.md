...
healthy files yielded nothing because one file confused one plugin, and the report was `ls: cannot
access 'out'` rather than a thinner IR.
...
Two shapes are answered. Two of one file's own Symbols under one id is the shape that happens,
because an id carries the file it came from, and the message names the id and both declarations'
lines — the id names the qualified name they share and nothing else tells them apart. An id an
earlier file already contributed is the shape only a plugin that wrote a path other than the
file's own can produce, and the message names the file that had it first; answering it here is
what leaves the document-wide check as a backstop rather than the first line of defence.
