...
A new `tag-action` job on the release workflow pushes two refs onto the release commit:

- `action-v<x.y.z>`, created once and never re-pointed, so a workflow pinned to it keeps
...
- `action-v<major>`, moved to the newest release of this action — the convenience alias,
...
against this package's real major (a `major` bump would strand every `@action-v0` in the
